...
function findFencedDiagram(text, cursor) {
  ...
  try {
    const fencedDiagrams = text.match(/\`\`\`mermaid[\s\S]*?\`\`\`/g).filter(diagram => {
      const start = text.indexOf(diagram);
      const end = start + diagram.length;
    ...
    console.log(fencedDiagrams);

    if (fencedDiagrams.length === 1) {
      return fencedDiagrams[0].slice(fenceStart.length, fencedDiagrams[0].length - fenceEnd.length);
    }
  } catch (e) {
    return void 0;
  }
