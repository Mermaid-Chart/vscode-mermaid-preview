...
let diagramMappings: {
  [key: string]: string[];
} = require("../src/diagramTypeWords.json");
  ...
  function updateMermaidChartTokenHighlighting() {
    ...
    if (activeEditor) {
      ...
      if (!isAuxFile(activeEditor.document.fileName)) {
        ...
        mermaidChartTokens = findMermaidChartTokensFromAuxFiles(
          activeEditor.document,
        );
      }
      ...
      (uuid: string) => {
        ...
        if (diagramCode) {
          ...
          vscode.window.showErrorMessage(
            "Diagram not found for the given UUID.",
          );
        }
      },
    ),
    ...
    vscode.commands.registerCommand(
      ...
      async (uri: vscode.Uri, range: vscode.Range) => {
        try {
          ...
          if (!mermaidCode) {
            vscode.window.showErrorMessage(
              "No valid Mermaid diagram found in the selected range.",
            );
            return;
          }
          ...
          vscode.window.showErrorMessage(
            `Error processing Mermaid diagram: ${error instanceof Error ? error.message : "Unknown error occurred."}`,
          );
        }
      },
    ),
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand(
      ...
      async (uri: vscode.Uri, range: vscode.Range) => {
        ...
        const response = await mcAPI.createDocumentWithDiagram(
          diagramCode,
          selectedProject.projectId,
        );

        const processedCode = ensureConfigBlock(
          diagramCode,
          response.documentID,
        );
        const editor = await await createMermaidFile(
          context,
          processedCode,
          true,
        );
  ...
  const syncFileToMermaidChart = async (document: vscode.TextDocument) => {
    if (document.languageId.startsWith("mermaid")) {
      ...
      try {
        const diagramId = extractIdFromCode(content);
        if (
          TempFileCache.hasTempUri(context, document.uri.toString()) &&
          diagramId
        ) {
          await mcAPI.saveDocumentCode(content, diagramId);
          ...
          vscode.window.showInformationMessage(
            "This is temporary buffer, this can not be saved locally",
          );
        } else if (
          !TempFileCache.hasTempUri(context, document.uri.toString()) &&
          diagramId
        ...
        vscode.window.showErrorMessage(
          `Failed to sync file: ${error instanceof Error ? error.message : "Unknown error occurred."}`,
        );
      }
    }
  };
