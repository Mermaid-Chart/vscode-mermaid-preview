...
export async function fetchMermaidChartProjects(): Promise<{ id: string; title: string }[]> {
  const token = getApiToken();
  const baseUrl = getBaseUrl();

  if (!token) {
    vscode.window.showErrorMessage('Mermaid Charts API token is not configured.');
    return [];
  }

  if (!baseUrl) {
    vscode.window.showErrorMessage('Mermaid Charts base URL is not configured.');
    return [];
  }
  try {
    ...
    if (axios.isAxiosError(error)) {
      console.error('Error fetching Mermaid Chart data:', error.message, error.request);
      ...
      if (error.response && error.response.status === 401) {
        ...
        vscode.window.showErrorMessage('Error fetching Mermaid Chart data: ' + error.message);
      }
...
export async function fetchMermaidChartDocuments(
  ...
  try {
    const response = await axios.get(`${baseUrl}/rest-api/projects/${projectID}/documents`, {
      headers: {
        Authorization: `Bearer ${token}`
      }
    });
